...
/* c = a + _s * b */
#define RATES_SUM_SCALED_2(_c, _a, _b, _num, _den) {		\
    (_c).p = (_a).p + ((_num)*(_b).p)/(_den);			\
    (_c).q = (_a).q + ((_num)*(_b).q)/(_den);			\
    (_c).r = (_a).r + ((_num)*(_b).r)/(_den);			\
  }

/* a = b * num / den*/
#define RATES_SCALE_2(_a, _b, _num, _den) {	\
    (_a).p = ((_b).p * (_num)) / (_den);		\
    (_a).q = ((_b).q * (_num)) / (_den);		\
    (_a).r = ((_b).r * (_num)) / (_den);		\
  }
