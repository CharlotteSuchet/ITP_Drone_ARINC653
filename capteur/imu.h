...
struct Imu {
  ...
  uint32_t           alt_us;           ///< altitude from Ultrasound sensor
  uint32_t           neutral_us;       ///< neutral ultrasound sensor
  uint32_t           alt_unscaled_us   ///< unscaled ultrasound mesurement
 };
...
struct ImuFloat {
  ...
  struct FloatEulers  eulers_CF;
  struct FloatEulers  eulers_prevLPF;
  struct FloatEulers  eulers_rad;
...
#define ImuScaleGyro(_imu) {					\
	...
	#define ImuScaleAccel(_imu) {					\
    	...
    	_imu.accel.z = (_imu.accel_unscaled.z - _imu.accel_neutral.z+512)*IMU_ACCEL_Z_SIGN; \     
	}
#endif

#ifndef ImuScaleUltrasound
	#define ImuScaleUltrasound(_imu) {									\
    	_imu.alt_us = abs((_imu.alt_unscaled_us - _imu.neutral_us)); \
  	}
#endif
